...
var openapiCmd = &cobra.Command{
	...
	Long: `Generate OpenAPI 3.1 specifications from your Nexo routes.
...
func runOpenAPIGenerate(cmd *cobra.Command, args []string) {
	...
	if openapiServerURL != "" {
		config.Servers = []nexo.OpenAPIServer{
	...
	// Create generator
	gen := nexo.NewOpenAPIGenerator(openapiAppDir, config)

	// Count routes
	scanner := nexo.NewScanner(openapiAppDir)

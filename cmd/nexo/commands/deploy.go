...
var deployCmd = &cobra.Command{
	Use:   "deploy",
	Short: "Build and deploy to Nexo Cloud",
	Long: `Build and deploy the current project to Nexo Cloud.

This command will:
1. Read nexo.yaml for app configuration
2. Build the Go binary (unless --no-build)
3. Build a Docker image
4. Push the image to GHCR
5. Trigger deployment on Nexo Cloud
...
func init() {
	...
	deployCmd.Flags().StringVar(&deployApp, "app", "", "App name (defaults to name in nexo.yaml)")
...
func runDeploy(cmd *cobra.Command, args []string) {
	...
	// Load nexo.yaml config
	v := viper.New()
	v.SetConfigName("nexo")
	...
	if err := v.ReadInConfig(); err != nil {
		if _, ok := err.(viper.ConfigFileNotFoundError); ok {
			...
			if appName == "" && !jsonOutput {
				form := huh.NewForm(
					huh.NewGroup(
						huh.NewInput().
							Title("App name").
							Description("Enter the name of your Nexo Cloud app").
							Value(&appName),
					),
				)
			...
			if jsonOutput {
				printJSONError(fmt.Errorf("failed to read nexo.yaml: %w", err))
			} else {
				fmt.Printf("  %s Failed to read nexo.yaml: %v\n", red("Error:"), err)
			}
			os.Exit(1)
		}
	} else {
	...
	if appName == "" {
		err := fmt.Errorf("app name required. Set 'name' in nexo.yaml or use --app flag")
		if jsonOutput {
			printJSONError(err)
		} else {
			fmt.Printf("  %s %v\n", red("Error:"), err)
...
`)
	...
	if !jsonOutput {
		fmt.Printf("  %s Deploying to Nexo Cloud...\n", yellow("->"))
	}

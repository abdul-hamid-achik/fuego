...
func TestContext_Query(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/test?name=nexo&count=5&active=true", nil)
	w := httptest.NewRecorder()
	c := NewContext(w, req)

	if c.Query("name") != "nexo" {
		t.Errorf("Expected 'nexo', got '%s'", c.Query("name"))
	}
...
func TestContext_Bind(t *testing.T) {
	...
	if data.Name != "nexo" {
		t.Errorf("Expected name 'nexo', got '%s'", data.Name)
	}
...
func TestContext_Store(t *testing.T) {
	...
	if c.Get("user") != "nexo" {
		t.Errorf("Expected 'nexo', got '%v'", c.Get("user"))
	}

	if c.GetString("user") != "nexo" {
		t.Errorf("Expected 'nexo', got '%s'", c.GetString("user"))
	}
...
func TestContext_RequestHelpers(t *testing.T) {
	...
	req.Header.Set("User-Agent", "NexoTest/1.0")
	...
	if c.UserAgent() != "NexoTest/1.0" {
		t.Errorf("Expected 'NexoTest/1.0', got '%s'", c.UserAgent())
	}
...
func TestContext_QueryDefault_WithValue(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/test?name=nexo", nil)
	w := httptest.NewRecorder()
	c := NewContext(w, req)

	result := c.QueryDefault("name", "default")
	if result != "nexo" {
		t.Errorf("expected 'nexo', got %q", result)
	}
}
...
func TestContext_Cookie(t *testing.T) {
	...
	req.AddCookie(&http.Cookie{Name: "user", Value: "nexo"})
	w := httptest.NewRecorder()
	c := NewContext(w, req)
	...
	if c.Cookie("user") != "nexo" {
		t.Errorf("expected 'nexo', got '%s'", c.Cookie("user"))
	}
}

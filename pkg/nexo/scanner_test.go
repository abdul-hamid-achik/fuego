...
func Put(c *nexo.Context) string {
	return ""
}

// Invalid: too many parameters
func Patch(c *nexo.Context, extra string) error {
	return nil
}
...
func TestScanner_ScanProxyInfo_ValidProxyWithContext(t *testing.T) {
	// Test with just "Context" (same package) instead of "nexo.Context"
	tmpDir := t.TempDir()
	appDir := filepath.Join(tmpDir, "app")
	if err := os.MkdirAll(appDir, 0755); err != nil {
		t.Fatalf("failed to create app dir: %v", err)
...
var ProxyConfig = nexo.ProxyConfig{
	Matcher: []string{
		"/api/*",
		"/admin/*",
	},
...
func TestScanner_ScanProxyInfo_WithMatchersPointer(t *testing.T) {
	...
	// ProxyConfig with pointer syntax (&nexo.ProxyConfig{})
	proxyContent := `package app
...
func TestScanner_ScanMiddlewareInfo_ValidSamePackage(t *testing.T) {
	// Test with just "MiddlewareFunc" instead of "nexo.MiddlewareFunc"
	tmpDir := t.TempDir()
	appDir := filepath.Join(tmpDir, "app")
	if err := os.MkdirAll(appDir, 0755); err != nil {
		t.Fatalf("failed to create app dir: %v", err)
...
func TestScanner_PathToLayoutPrefix(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "route group with nested",
			filePath: "app/(dashboard)/settings/layout.templ",
			want:     "/settings",
		},
		{
			name:     "deeply nested",
...
func TestScanner_ScanPageInfo_MultipleDynamicPages(t *testing.T) {
	...
	// Create multiple pages with dynamic segments using bracket convention
	dirs := []string{

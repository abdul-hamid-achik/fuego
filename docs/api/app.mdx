...
The `App` struct is the core of a Nexo application. It manages routing, middleware, server lifecycle, and configuration.
...
<AccordionGroup>
  <Accordion title="Routing Methods" icon="route" defaultOpen>
    ...
    ```go
    app.Get("/users", func(c *nexo.Context) error {
    ...
    ```go
    app.Post("/users", func(c *nexo.Context) error {
        var user User
        if err := c.Bind(&user); err != nil {
            return nexo.BadRequest("invalid body")
        }
    ...
    ```go
    app.Delete("/users/{id}", func(c *nexo.Context) error {
        id := c.Param("id")
        // Delete user...
        return c.NoContent()
    })
...
```go
app.Group("/api/v1", func(v1 *nexo.RouteGroup) {
    v1.Use(nexo.Logger())
    
    v1.Group("/users", func(users *nexo.RouteGroup) {
    ...
    v1.Group("/posts", func(posts *nexo.RouteGroup) {
        posts.Get("/", listPosts)
...
func main() {
    ...
    // API routes
    app.Group("/api", func(api *nexo.RouteGroup) {
        api.Get("/health", func(c *nexo.Context) error {

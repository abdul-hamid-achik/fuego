---
title: Context API
description: 'Complete reference for the nexo.Context object passed to every handler.'
---

The `nexo.Context` is passed to every handler and provides access to request data and response methods.
...
```go
// Route: /api/users/_id/route.go → /api/users/:id
...
func Post(c *nexo.Context) error {
    ...
    if err != nil {
        return nexo.BadRequest("file required")
    }
    defer file.Close()
...
func Get(c *nexo.Context) error {
    ...
    if sessionID == "" {
        return nexo.Unauthorized("no session")
    }
...
func Get(c *nexo.Context) error {
    ...
    // 401 Unauthorized
    return nexo.Unauthorized("login required")

    // 403 Forbidden
    return nexo.Forbidden("access denied")
...
func GetByID(c *nexo.Context) error {
    id := c.ParamInt("id")
    if id == 0 {
        return nexo.BadRequest("invalid user ID")
    }
...
func Delete(c *nexo.Context) error {
    ...
    if currentUserID != id && !c.GetBool("is_admin") {
        return nexo.Forbidden("cannot delete other users")
    }

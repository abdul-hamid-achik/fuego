---
title: OpenAPI
description: 'Automatic OpenAPI specification generation for your Nexo APIs'
---

Nexo automatically generates OpenAPI 3.1 specifications from your file-based routes, enabling interactive API documentation with Swagger UI.
...
Nexo extracts documentation from your code automatically:
...
| `app/api/posts/_id/route.go` | `["posts"]` |

Dynamic segments like `_id` and route groups like `_group_admin` are ignored when deriving tags.
...
For POST, PUT, and PATCH methods, Nexo automatically adds request body schemas:
...
- **Swagger UI** - Interactive API explorer (built-in with `nexo openapi serve`)
...
You can also configure OpenAPI generation in `nexo.yaml`:
...
```bash
nexo openapi generate  # Reads from nexo.yaml
```
...
<AccordionGroup>
  ...
  <Accordion title="4. Version your API">
    ...
    ```bash
    nexo openapi generate --version 2.0.0
    ```
  </Accordion>
...
<AccordionGroup>
  <Accordion title="OpenAPI spec is empty">
    ...
    **Solution:** 
    ...
    - Run `nexo routes` to see if routes are detected
  </Accordion>
  ...
  <Accordion title="Wrong tags generated">
    ...
    **Solution:**
    ...
    - Dynamic segments `_id` and groups `_group_name` are ignored
  </Accordion>

---
title: Familiar Patterns
description: 'If you know Next.js, Nuxt, or SvelteKit, you already know Nexo.'
---

If you've used modern meta-frameworks like Next.js, Nuxt, SvelteKit, or Remix, Nexo's conventions will feel natural.

## Quick Comparison

| Concept | Other Frameworks | Nexo |
|---------|------------------|-------|
| Route file | `route.ts` / `+server.ts` | `route.go` |
| Page file | `page.tsx` / `+page.svelte` | `page.templ` |
| Layout | `layout.tsx` / `+layout.svelte` | `layout.templ` |
| Middleware | `middleware.ts` | `middleware.go` |
| Dynamic segment | `[id]` | `_id` |
| Catch-all | `[...slug]` | `__slug` |
| Optional catch-all | `[[...slug]]` | `___slug` |
| Route groups | `(group)` | `_group_name` |

<Info>
Nexo uses underscores instead of brackets to ensure all directories are valid Go package names. No symlinks or special import handling required!
</Info>

## Key Differences

### Handlers Are Named After HTTP Methods

In JavaScript frameworks, you export named functions like `GET`, `POST`, etc. In Nexo, you define functions with capitalized method names:

```go
// app/api/users/route.go
package users

import "github.com/abdul-hamid-achik/nexo/pkg/nexo"

func Get(c *nexo.Context) error {
    return c.JSON(200, []string{"alice", "bob"})
}

func Post(c *nexo.Context) error {
    var user User
    if err := c.Bind(&user); err != nil {
        return nexo.BadRequest("invalid input")
    }
    return c.JSON(201, user)
}

func Put(c *nexo.Context) error { /* ... */ }
func Patch(c *nexo.Context) error { /* ... */ }
func Delete(c *nexo.Context) error { /* ... */ }
```

### Templates Use templ, Not JSX

Instead of JSX/TSX, Nexo uses [templ](https://templ.guide) for type-safe HTML:

```go
// app/dashboard/page.templ
package dashboard

templ Page() {
    <div class="container">
        <h1>Dashboard</h1>
        <p>Welcome back!</p>
    </div>
}
```

Templ provides:
- Compile-time type checking
- No runtime template parsing
- Go code completion in templates
- Smaller binary size than text/template

### No Build Step for Go Code

Unlike JavaScript frameworks that require bundling, Nexo compiles to a single binary:

```bash
# Development
nexo dev

# Production
nexo build
./myapp
```

The binary includes everything - no need for `node_modules`, npm, or a separate build step for your Go code.

### HTMX Instead of Client-Side Frameworks

For interactivity, Nexo encourages HTMX over React/Vue/Svelte:

```html
<!-- Load data on page load -->
<div hx-get="/api/users" hx-trigger="load">
    Loading...
</div>

<!-- Submit form without page reload -->
<form hx-post="/api/users" hx-target="#user-list">
    <input name="name" />
    <button type="submit">Add User</button>
</form>
```

Benefits:
- No JavaScript to write
- Server renders HTML
- Smaller page weight
- Works with any backend

## Migration Patterns

### From Next.js App Router

| Next.js | Nexo |
|---------|-------|
| `app/api/users/route.ts` | `app/api/users/route.go` |
| `export async function GET()` | `func Get(c *nexo.Context) error` |
| `NextRequest` | `*nexo.Context` |
| `NextResponse.json()` | `c.JSON(status, data)` |
| `NextResponse.redirect()` | `c.Redirect(status, url)` |
| `cookies().get()` | `c.Cookie("name")` |
| `headers().get()` | `c.Header("name")` |

### From Nuxt

| Nuxt | Nexo |
|------|-------|
| `server/api/users.ts` | `app/api/users/route.go` |
| `defineEventHandler()` | `func Get(c *nexo.Context) error` |
| `getQuery(event)` | `c.Query("key")` |
| `readBody(event)` | `c.Bind(&body)` |
| `setResponseStatus()` | `c.JSON(status, data)` |

### From SvelteKit

| SvelteKit | Nexo |
|-----------|-------|
| `+server.ts` | `route.go` |
| `+page.svelte` | `page.templ` |
| `+layout.svelte` | `layout.templ` |
| `RequestHandler` | `func Get(c *nexo.Context) error` |
| `json()` helper | `c.JSON(status, data)` |

## File Structure Comparison

### JavaScript Framework (Next.js style)

<FileTree>
  <Folder name="app" defaultOpen>
    <Folder name="api">
      <Folder name="users">
        <Folder name="[id]">
          <File name="route.ts" />
        </Folder>
      </Folder>
    </Folder>
    <Folder name="dashboard">
      <File name="page.tsx" />
      <File name="layout.tsx" />
    </Folder>
    <File name="layout.tsx" />
  </Folder>
</FileTree>

### Nexo

<FileTree>
  <Folder name="app" defaultOpen>
    <Folder name="api">
      <Folder name="users">
        <Folder name="_id">
          <File name="route.go" />
        </Folder>
      </Folder>
    </Folder>
    <Folder name="dashboard">
      <File name="page.templ" />
      <File name="layout.templ" />
    </Folder>
    <File name="layout.templ" />
  </Folder>
</FileTree>

The structure is similar - swap extensions and use underscores for dynamic segments.

## Code Comparison

<Tabs>
  <Tab title="Next.js">
```typescript
// app/api/users/route.ts
import { NextRequest, NextResponse } from 'next/server'

export async function GET(request: NextRequest) {
  const users = await db.users.findMany()
  return NextResponse.json(users)
}

export async function POST(request: NextRequest) {
  const body = await request.json()
  const user = await db.users.create({ data: body })
  return NextResponse.json(user, { status: 201 })
}
```
  </Tab>
  <Tab title="Nexo">
```go
// app/api/users/route.go
package users

import "github.com/abdul-hamid-achik/nexo/pkg/nexo"

func Get(c *nexo.Context) error {
    users := db.Users.FindMany()
    return c.JSON(200, users)
}

func Post(c *nexo.Context) error {
    var body CreateUserInput
    if err := c.Bind(&body); err != nil {
        return nexo.BadRequest("invalid input")
    }
    user := db.Users.Create(body)
    return c.JSON(201, user)
}
```
  </Tab>
</Tabs>

## Why Go?

If you're comfortable with JavaScript frameworks, why use Nexo?

| Benefit | Description |
|---------|-------------|
| **Performance** | Go is significantly faster than Node.js for CPU-bound tasks |
| **Memory** | Lower memory footprint, important for containers |
| **Type safety** | Compile-time checks catch errors early |
| **Deployment** | Single binary, no runtime dependencies |
| **Concurrency** | Goroutines handle concurrent requests efficiently |
| **Simplicity** | No package manager drama, stable stdlib |

## Next Steps

<CardGroup cols={2}>
  <Card title="Quick Start" icon="rocket" href="/docs/getting-started/quickstart">
    Get up and running
  </Card>
  <Card title="File-Based Routing" icon="folder-tree" href="/docs/routing/file-based">
    Deep dive into routing
  </Card>
  <Card title="Context API" icon="code" href="/docs/api/context">
    Request and response handling
  </Card>
  <Card title="Templates" icon="file-code" href="/docs/core-concepts/templates">
    Learn templ for pages
  </Card>
</CardGroup>

...
description: 'Connect your Nexo application to databases using GORM, sqlc, or raw SQL with the repository pattern.'
---

Nexo is database-agnostic and works with any Go database library. This guide covers the most popular approaches: GORM (ORM), sqlc (type-safe SQL), and the standard `database/sql` package.
...
func Connect() error {
    ...
    if dsn == "" {
        dsn = "host=localhost user=postgres password=postgres dbname=nexo_app port=5432 sslmode=disable"
    }

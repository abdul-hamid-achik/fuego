...
This guide provides comprehensive, production-ready examples for building applications with Nexo. Each example is complete and can be used as a starting point for your projects.
...
### Catch-All Route `__slug`

```go
// app/api/posts/__slug/route.go
package __slug
...
### Optional Catch-All `___path`

```go
// app/api/docs/___path/route.go
package ___path
...
// GET /api/docs/___path - Optional catch-all for documentation
// Matches:
//   /api/docs                    → path = ""
...
### Route Groups `_group_name`

```go
// app/api/_group_admin/settings/route.go
package settings

import "github.com/abdul-hamid-achik/nexo/pkg/nexo"

// GET /api/settings - The _group_admin group doesn't affect the URL
func Get(c *nexo.Context) error {
    return c.JSON(200, map[string]any{
        "theme":          "dark",
        "notifications":  true,
...
```go
// app/api/_group_admin/dashboard/route.go
package dashboard
...
var Default = &TaskStore{
    tasks: []Task{
        {ID: 1, Title: "Learn Nexo", Completed: true},
...
```bash
nexo tailwind build  # For production (minified)
nexo tailwind watch  # For development
```
...
func JWTMiddleware() nexo.MiddlewareFunc {
    return func(next nexo.HandlerFunc) nexo.HandlerFunc {
        return func(c *nexo.Context) error {
...
func RequireRole(roles ...string) nexo.MiddlewareFunc {
    return func(next nexo.HandlerFunc) nexo.HandlerFunc {
        return func(c *nexo.Context) error {
...
func Proxy(c *nexo.Context) (*nexo.ProxyResult, error) {
    ...
    if path == "/pricing" {
        ...
        if variant == "B" {
            return nexo.Rewrite("/pricing-new").
...
func Proxy(c *nexo.Context) (*nexo.ProxyResult, error) {
    ...
    if isEUCountry(country) && strings.HasPrefix(path, "/api/") {
        return nexo.Rewrite("/api/eu" + strings.TrimPrefix(path, "/api")).
    ...
    if path == "/" {
        switch country {
        case "DE":
            return nexo.Redirect("/de", 302), nil
        case "FR":
            return nexo.Redirect("/fr", 302), nil
        case "ES":
            return nexo.Redirect("/es", 302), nil
        }
    }
...
<AccordionGroup>
  ...
  <Accordion title="Error Handling">
    ...
    - Use `nexo.BadRequest()`, `nexo.Unauthorized()`, etc.

...
Nexo provides a structured error handling system that makes it easy to return consistent error responses and handle errors gracefully throughout your application.

## HTTPError Type

The foundation of Nexo's error handling is the `HTTPError` type:
...
When you return an `HTTPError` from a handler, Nexo automatically serializes it to a consistent JSON response:
...
Nexo provides convenience functions for common HTTP errors:
...
// 403 Forbidden
return nexo.Forbidden("insufficient permissions")

// 404 Not Found
return nexo.NotFound("user not found")

// 409 Conflict
return nexo.Conflict("email already exists")
```
  </Tab>
  <Tab title="Server Errors (5xx)">
```go
// 500 Internal Server Error
return nexo.InternalServerError("database connection failed")

// Custom error with any status code
return nexo.NewHTTPError(503, "service temporarily unavailable")
```
  </Tab>
</Tabs>
...
func Get(c *nexo.Context) error {
    id := c.Param("id")
    if id == "" {
        return nexo.BadRequest("user ID is required")
    }
...
func Put(c *nexo.Context) error {
    ...
    if err := c.Bind(&input); err != nil {
        return nexo.BadRequest("invalid JSON body")
    }

    // Validate input
    if input.Name == "" {
        return nexo.BadRequest("name is required")
    }
    if !isValidEmail(input.Email) {
        return nexo.BadRequest("invalid email format")
    }

    // Check for conflicts
    existing, _ := db.FindUserByEmail(input.Email)
    if existing != nil && existing.ID != id {
        return nexo.Conflict("email already in use")
    }

    user, err := db.UpdateUser(id, input)
    if err != nil {
        return nexo.InternalServerError("failed to update user")
    }
...
func Get(c *nexo.Context) error {
    ...
    if err != nil {
        ...
        wrappedErr := nexo.WrapError(err, "failed to fetch external data")
        log.Printf("error: %v", wrappedErr)
        
        return nexo.InternalServerError("external service unavailable")
    }
...
func Get(c *nexo.Context) error {
    ...
    if err != nil {
        ...
        return nexo.NewHTTPErrorWithCause(
            500,
...
// main.go
app := nexo.New()
app.Use(nexo.Recover()) // Catches panics, returns 500
```
...
func (i *CreateUserInput) Validate() error {
    ...
    if len(i.Name) < 2 {
        return nexo.BadRequest("name must be at least 2 characters")
    }
    if !isValidEmail(i.Email) {
        return nexo.BadRequest("invalid email format")
    }
    if len(i.Password) < 8 {
        return nexo.BadRequest("password must be at least 8 characters")
    }
    return nil
}
...
if err != nil {
    ...
    return nexo.InternalServerError("failed to process request") // Generic message
}
```
  </Accordion>

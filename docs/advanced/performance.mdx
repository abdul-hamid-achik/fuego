---
title: Performance
description: 'Optimize your Nexo application for maximum throughput and minimal latency.'
---

Nexo is built on Go's high-performance `net/http` and `chi` router, providing excellent performance out of the box. This guide covers techniques to optimize your application further.

## Performance Characteristics

Nexo inherits Go's performance benefits:
...
func Get(c *nexo.Context) error {
    ...
    if err != nil {
        return nexo.InternalServerError("failed to fetch users")
    }
...
func Timing() nexo.MiddlewareFunc {
...
func SlowMiddleware() nexo.MiddlewareFunc {
    return func(next nexo.HandlerFunc) nexo.HandlerFunc {
        return func(c *nexo.Context) error {
...
```go
// In Nexo - OK for moderate traffic
...
func main() {
    ...
    app.Get("/debug/pprof/*", func(c *nexo.Context) error {
...
func Metrics() nexo.MiddlewareFunc {
...
Nexo apps are stateless by default, making horizontal scaling easy:

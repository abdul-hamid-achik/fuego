---
title: Testing
description: 'Write comprehensive tests for your Nexo applications with built-in testing utilities.'
---

Nexo applications are easy to test using Go's standard `testing` package and `net/http/httptest`. This guide covers testing patterns for handlers, middleware, and full integration tests.

## Testing Setup

Nexo provides utilities that work seamlessly with Go's testing infrastructure:
...
func TestGetUsers(t *testing.T) {
    ...
    app.Get("/api/users", func(c *nexo.Context) error {
...
func TestCreateUser(t *testing.T) {
    ...
    app.Post("/api/users", func(c *nexo.Context) error {
        var input struct {
            Name  string `json:"name"`
            Email string `json:"email"`
        }
        if err := c.Bind(&input); err != nil {
            return nexo.BadRequest("invalid JSON")
        }
...
func TestGetUserByID(t *testing.T) {
    ...
    app.Get("/api/users/{id}", func(c *nexo.Context) error {
        id := c.Param("id")
...
func TestUserEndpoints(t *testing.T) {
    ...
    app.Put("/api/users/{id}", func(c *nexo.Context) error {
        return c.JSON(200, map[string]string{"action": "update"})
    })
    app.Delete("/api/users/{id}", func(c *nexo.Context) error {
        return c.NoContent()
    })
...
func TestAuthMiddleware(t *testing.T) {
    authMiddleware := func(next nexo.HandlerFunc) nexo.HandlerFunc {
        return func(c *nexo.Context) error {
            ...
            if token != "Bearer valid-token" {
                return nexo.Forbidden("invalid token")
            }
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            app.Get("/api/protected", func(c *nexo.Context) error {
...
func TestMiddlewareOrder(t *testing.T) {
    ...
    mw1 := func(next nexo.HandlerFunc) nexo.HandlerFunc {
        return func(c *nexo.Context) error {
    ...
    mw2 := func(next nexo.HandlerFunc) nexo.HandlerFunc {
        return func(c *nexo.Context) error {
    ...
    app.Use(mw1)
    app.Use(mw2)
    app.Get("/test", func(c *nexo.Context) error {
...
func TestErrorResponses(t *testing.T) {
    ...
    app.Get("/api/bad-request", func(c *nexo.Context) error {
        return nexo.BadRequest("invalid input")
    })
    
    app.Get("/api/not-found", func(c *nexo.Context) error {
        return nexo.NotFound("resource not found")
    })
    
    app.Get("/api/panic", func(c *nexo.Context) error {
        panic("something went wrong")
    })
    
    app.Use(nexo.Recover()) // Add recover middleware
    app.Mount()
...
func TestUserCRUD_Integration(t *testing.T) {
    ...
    app.Post("/api/users", func(c *nexo.Context) error {
        ...
        if err != nil {
            return nexo.InternalServerError("failed to create user")
        }
...
// TestApp wraps nexo.App with testing utilities
type TestApp struct {
    *nexo.App
    t *testing.T
}
...
func TestHTMXHandler(t *testing.T) {
    ...
    app.Get("/api/items", func(c *nexo.Context) error {
        if c.IsHTMX() {
...
`
    ...
    app := nexo.New(nexo.WithAppDir(filepath.Join(tmpDir, "app")))

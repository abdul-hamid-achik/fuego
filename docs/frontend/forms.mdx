...
Learn how to handle HTML forms in Nexo, including validation, HTMX integration, and file uploads.
...
func Post(c *nexo.Context) error {
    ...
    if name == "" || email == "" {
        return nexo.BadRequest("name and email are required")
    }
...
func Post(c *nexo.Context) error {
    ...
    if !strings.HasPrefix(contentType, "image/") {
        return nexo.BadRequest("only images allowed")
    }
    
    // Validate file size (e.g., 5MB max)
    if header.Size > 5*1024*1024 {
        return nexo.BadRequest("file too large (max 5MB)")
    }
    
    // Save file
    dst, err := os.Create("uploads/" + header.Filename)
    if err != nil {
        return nexo.InternalServerError("could not save file")
    }
    defer dst.Close()
...
func Post(c *nexo.Context) error {
    ...
    if err != nil {
        return nexo.BadRequest("invalid form")
    }

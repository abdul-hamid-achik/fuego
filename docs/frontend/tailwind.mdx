...
Nexo includes a standalone Tailwind CSS v4 binary, so you can use Tailwind without Node.js or npm.

## Setup

When creating a new project with `nexo new`:
...
You'll see:
```
[nexo] Starting development server...
[tailwind] Watching for changes...
[nexo] Server running at http://localhost:3000
```
...
# Install Nexo for Tailwind
RUN go install github.com/abdul-hamid-achik/nexo/cmd/fuego@latest
...
<AccordionGroup>
  <Accordion title="CSS not updating">
    Make sure `nexo tailwind watch` is running, or run `nexo tailwind build` after changes.
  </Accordion>
  ...
  <Accordion title="Binary not found">
    Run `nexo tailwind install` to download the Tailwind binary.
  </Accordion>
</AccordionGroup>
